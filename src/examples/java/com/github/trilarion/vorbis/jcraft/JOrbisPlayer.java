 ...
 * provided here for setting an example, how to use JOrbis for editing comments.
 *
 * The following command will generate new Ogg Vorbis file 'bar.ogg' from
 * 'foo.ogg' and 'bar.ogg' will include comments from 'foo.ogg' and new two
 * comments; TITLE, ARTIST.
 *
 * $ java -classpath .. -DJOrbis.comment.0=TITLE=sometitle \
 * -DJOrbis.comment.1=ARTIST=somename \ JOrbisComment foo.ogg bar.ogg
 *
 */
public class JOrbisPlayer extends JApplet implements ActionListener, Runnable {
    ...
    static final int convsize = BUFSIZE * 2;
        ...
        while (true) {
...

            {
                ...
                for (byte[] ptr1 : ptr) {
                    ...
                    System.err.println("Comment: " + new String(ptr1, 0, ptr1.length - 1));
                    if (sb != null) {
                        sb.append(" ").append(new String(ptr1, 0, ptr1.length - 1));
                    }
                }
            ...
            // convsize = BUFSIZE / vi.channels; // is this right, than it should be differently initialized
